...
```ts
import type { PaletteEntry, SlotMap } from "../dsl/types.js";
...
export function serializeCustomerTheme(pair: DerivedPair): {
  ...
  const source = `import type { Palette, SlotMap } from "../../dsl/types.js";
...
`;
  ...
  const registration = `  ${vector.name}: { palette: ${id}Palette, slots: ${id}Slots, componentOverrides: ${id}ComponentOverrides },
  "${vector.name}-dark": { palette: ${id}Palette, slots: ${id}DarkSlots, base: "dark", componentOverrides: ${id}ComponentOverrides },`;
