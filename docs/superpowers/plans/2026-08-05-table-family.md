...
- **Four gates, not three:**
  ...
  ```
- **Every task runs `pnpm build` before committing — not just `test` and `lint`.** `pnpm build` runs `tsc`, and it is the only gate that catches a type error. Task 5 shipped a `process.env.NODE_ENV` reference with no `@types/node` in this package; `pnpm test` and `pnpm lint` both passed green while `pnpm build` was broken, and the breakage only surfaced during Task 6 — where it also cascaded into 7 unrelated test failures. A green test+lint pair says nothing about the build.
