...
import {
  ...
  Tab, TabList, TabPanel, Tabs, Tag, Toolbar, useToast,
...
// tabbed-workspace: saved views. Declared once so the tabs and the panels stay
// in step — a tab whose value has no panel would leave aria-controls dangling.
const VIEWS = [
  { value: "all", label: "All" },
...
export function TransactionsScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    const rows = TRANSACTIONS.filter((t) => {
      ...
      const matchesView =
        view === "all" ||
        (view === "supplies" && t.category === "Supplies") ||
        (view === "large" && Math.abs(t.amount) >= 1000);
      return matchesQuery && matchesCategory && matchesView && !voided.has(t.id);
    });
    if (!sort) return rows;
  ...
  }, [query, category, sort, voided, view]);
  ...
  return (
    <main className="psi-container">
      ...
      >
        ...
        {/* One panel per tab so every aria-controls resolves. Only the active
            one is filled: every TabPanel renders (D67), so putting the table in
            all three would mount it three times. This is the "render less
            inside the panel" the spec points at. */}
        {VIEWS.map((v) => (
